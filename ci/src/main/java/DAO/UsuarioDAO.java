...
public class UsuarioDAO {
	public void salvar(Usuario u) throws SQLException {
		...
		try {
			...
			System.out.println("N�o foi possivel inserir!" + e.getMessage());
		}
	...
	public static void main(String[] args) {
		/*
		 * user.setIdUsuario(6L); user.setTipoUsuario(10);
		 * user.setNome("manoela"); user.setLogin("manu"); user.setSenha("123");
		 * 
		 * UsuarioDAO dao = new UsuarioDAO();
		 * 
		 * try { dao.editar(user); System.out.println(
		 * "Secretaria editado com sucesso"); } catch (SQLException ex) {
		 * System.out.println("n�o foi possivel editar a Secretaria " +
		 * ex.getMessage()); } Usuario user = new Usuario();
		 * user.setTipoUsuario(10);
		 * 
		 * 
		 * UsuarioDAO dao = new UsuarioDAO();
		 * 
		 * try { ArrayList<Usuario> lista = dao.buscarPorTipo(30); for(Usuario u
		 * : lista ) { System.out.println("Resultado: "+u); }
		 * 
		 * } catch (SQLException ex) { System.out.println(
		 * "n�o foi possivel buscar a Secretaria " + ex.getMessage()); }
		 */
